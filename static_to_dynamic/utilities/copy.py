...
def copyAllfiles(staticPath: str, HtmlDir: str, mainPath: str, htmlPath: str, cssPath: str) -> bool:
    try:
        imgPath = os.path.join(staticPath, 'images')
        jsPath = os.path.join(staticPath, 'js')
        webfontPath = os.path.join(staticPath, 'webfonts')
        otherPath = os.path.join(staticPath, 'others')

        class fileSearcher():

            def __init__(self, ext, dotext):
                self.ext = glob.glob(
                    HtmlDir + '/**/*.{}'.format(ext), recursive=True)
                self.dotext = dotext
